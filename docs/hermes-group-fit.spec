# Discord Friend Bot Hermes + Group Fit Learning Spec

## Overview

Discord Friend Bot runs as a Python Discord orchestrator plus Hermes Agent as a subprocess.
...
on_message
  ...
  -> build Hermes prompt:
       ...
       safety instructions
  -> hermes --profile discord-bot ... chat -q "<prompt>"
...
│   └── topic_log.py     # new
└── docs/
    └── discord-bot-hermes-group-fit.spec
```
...
```text
hermes --profile discord-bot --toolsets <toolset> chat -q "<prompt>"
```
...
```text
STYLE_GUIDE_DB_PATH=/data/bot_style.db
```
...
```text
TOPIC_LOG_DB_PATH=/data/bot_topics.db
```
