# Discord Friend Bot Spec — Orchestrator + Hermes Architecture

## Overview

Discord Friend Bot is a Discord LLM bot that runs as a **Python orchestrator** (this repo) + **Hermes Agent** (subprocess). The orchestrator handles Discord connectivity, permissions, and state tracking. Hermes is only the brain — it gets text in, gives text out.

**Rule: NEVER modify Hermes. Treat it as a black box CLI.**

Repo: `github.com/your-org/discord-friend-bot`
...
Permission gate → classify user:
  - Admin (in ADMIN_USER_IDS) → full Hermes tools
  - Approved (in APPROVED_USER_IDS) → restricted tools
...
Autonomous state check:
  ...
  - Decide: reply, react, spontaneous join, or silent
    ↓
If Hermes needed:
  spawn: hermes --profile discord-bot --toolsets WEB,image_gen chat -q "..."
...
│   └── profile/             # Template copied to ~/.hermes/profiles/discord-bot/
...
class BotConfig:
    ...
    def __post_init__(self):
        ...
        raw_approved = os.getenv("APPROVED_USER_IDS", "")
...
DB_PATH = os.getenv("STATE_DB_PATH", "/data/bot_state.db")
...
Creates the Discord bot profile on first run.
...
PROFILE_DIR = os.path.join(HERMES_HOME, "profiles", "discord-bot")

def ensure_discord_bot_profile(config):
    """Create the Discord bot Hermes profile if it doesn't exist."""
    if os.path.exists(PROFILE_DIR):
        logger.info("Discord Friend Bot profile already exists")
        return

    # Create profile by cloning from active profile
    result = subprocess.run(
        ["hermes", "profile", "create", "discord-bot", "--clone"],
        capture_output=True, text=True
    )
    if result.returncode != 0:
        logger.error(f"Failed to create Discord bot profile: {result.stderr}")
        return

    # Set model
    subprocess.run([
        "hermes", "-p", "discord-bot", "config", "set",
    ...
    if config.openrouter_api_key:
        with open(env_path, "a") as f:
            if "LLM_API_KEY" not in open(env_path).read():
    ...
    # Install Discord bot persona skill
    skill_dir = os.path.join(PROFILE_DIR, "skills", "discord-bot-persona")
    ...
    logger.info("Discord Friend Bot profile created successfully")

DISCORD_BOT_PERSONA_SKILL = """---
name: discord-bot-persona
description: "Discord personality rules for this Discord bot."
---

# Discord Friend Bot Discord Persona

You are responding to Discord messages. Follow these rules:

## Identity
- You are the Discord bot. You hang out in a friend group Discord server.
...
#!/usr/bin/env python3
"""
Discord Friend Bot Orchestrator — Discord bot + Hermes Agent subprocess integration.
"""
...
from hermes.setup import ensure_discord_bot_profile
...
class DiscordLLMBot:
    ...
    async def on_ready(self):
        ...
        await asyncio.get_event_loop().run_in_executor(None, ensure_discord_bot_profile, self.config)
    ...
    def _build_prompt(self, content: str, context: str, permission: PermissionLevel, state: dict) -> str:
        ...
        parts = [
            "You are the Discord bot in this friend group server.",
            "",
            "Recent conversation:",
...
APPROVED_USER_IDS=             # Comma-separated Discord user IDs

# OpenRouter / LLM
LLM_API_KEY=                   # API key for Hermes LLM
...
Hermes is a separate installation at `~/.hermes/`. The orchestrator creates a `discord-bot` profile there on first run via `hermes profile create discord-bot --clone`. The profile's SOUL.md is the original Hermes voice. Discord personality is in the `discord-bot-persona` skill.

The orchestrator calls:
```
hermes --profile discord-bot --toolsets WEB,image_gen chat -q "prompt goes here"
```
