...
- Spontaneous reacts/replies are per-user opt-in and default off; channel controls can further tune rates and modes
...
TRIGGER_DEFAULTS_CSV=trigger_defaults.csv
...
For an Oracle Cloud Always Free VM deployment, see [docs/oracle-cloud-deploy.md](docs/oracle-cloud-deploy.md).
...
- `!ping`
- `!whoami`
- `!highlights [scan_limit]`
- `!trigger <text> [response]` or attach an image with `!trigger <text>`
- `!forget <text>`
- `!improve <suggestion>`
- `!control status|mute|unmute|prompted|strict|normal|spontaneous|remember|forget|privacy`

Developer/admin:

- `!admin ...` for local diagnostics and maintenance
- `!admin ask <request>` routes an admin-only request to the optional Hermes CLI harness when `HERMES_ENABLED=true`
...
- Admin/control commands must be protected by `ADMIN_USER_IDS` and/or `CONTROL_ADMIN_IDS`.
- Hermes harness access is disabled by default and should only be enabled for trusted admins; `HERMES_ADMIN_TOOLSETS` may include powerful local tools.
