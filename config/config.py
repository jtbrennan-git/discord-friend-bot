"""
Configuration management for Discord Friend Bot.
"""
...
class BotConfig:
    ...
    def __init__(self):
        ...
        self.trigger_defaults_csv: str = os.getenv("TRIGGER_DEFAULTS_CSV", "trigger_defaults.csv").strip()
        self.trigger_defaults_import_enabled: bool = os.getenv("TRIGGER_DEFAULTS_IMPORT_ENABLED", "true").lower() == "true"

        # Optional Hermes CLI harness. Disabled by default because admin toolsets may be powerful.
        self.hermes_enabled: bool = os.getenv("HERMES_ENABLED", "false").lower() == "true"
        self.hermes_binary: str = os.getenv("HERMES_BINARY", "hermes").strip() or "hermes"
        self.hermes_profile: str = os.getenv("HERMES_PROFILE", "discord-bot").strip() or "discord-bot"
        self.hermes_provider: str = os.getenv("HERMES_PROVIDER", "openrouter").strip()
        self.hermes_model: str = os.getenv("HERMES_MODEL", self.llm_model).strip()
        self.hermes_admin_toolsets: str = os.getenv(
            "HERMES_ADMIN_TOOLSETS",
            "all",
        ).strip()
        self.hermes_timeout_seconds: float = float(os.getenv("HERMES_TIMEOUT_SECONDS", "60"))
        self.hermes_max_output_chars: int = int(os.getenv("HERMES_MAX_OUTPUT_CHARS", "4000"))
        ...
        if self.hermes_timeout_seconds <= 0:
            raise ValueError("HERMES_TIMEOUT_SECONDS must be a positive number")
        if self.hermes_max_output_chars <= 0:
            raise ValueError("HERMES_MAX_OUTPUT_CHARS must be a positive integer")
