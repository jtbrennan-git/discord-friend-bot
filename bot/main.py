#!/usr/bin/env python3
"""
Main bot application for Discord Friend Bot.
...
from utils.hermes import HermesClient

IMPROVEMENTS_LOG = os.getenv("IMPROVEMENTS_LOG", "/tmp/bot_improvements.log")
BOT_LOG_PATH = os.getenv("BOT_LOG_PATH", "/tmp/bot.log")
...
class DiscordLLMBot:
    ...
    async def on_message(self, message: discord.Message):
        ...
        if is_reply_to_bot:
            ...
            if await self._maybe_process_direct_command(message, content):
                return
    ...
    async def _generate_and_execute(self, message: discord.Message, content: str,
        ...
        try:
            ...
            if parsed.action == "REPLY" and self._looks_like_refusal(parsed.content):
                parsed = await self._retry_refusal_reply(
                    original_content=content,
                    refused_text=parsed.content,
                    system_prompt=system_prompt,
                    context=context,
    ...
    async def _retry_refusal_reply(
        self,
        ...
        retry_prompt = (
            "Your previous draft sounded like a generic assistant refusal, which does not fit this bot.\n"
            "You are in a casual Discord friend-group context, not a customer support chat. "
            "Stay within normal safety boundaries, but do not perform corporate refusal language, "
            "do not say you are an AI/language model, and do not claim you cannot participate just because the request is casual, edgy, or conversational.\n"
            "If a direct answer is genuinely unsafe or you have nothing useful to add, choose [SILENT]. "
            "Otherwise answer naturally in the local style.\n\n"
            f"Original message:\n{original_content or 'React to the chat.'}\n\n"
            f"Refusal draft to replace:\n{refused_text}"
        )
        raw = await self._llm_generate(
            retry_prompt,
            system_prompt=system_prompt,
    ...
    async def _maybe_process_direct_command(self, message: discord.Message, content: str) -> bool:
    ...
    def _looks_like_refusal(text: str) -> bool:
        ...
        if not normalized:
            return False
        refusal_patterns = [
            r"\bi (?:can(?:not|'t)|won't|am unable to|cannot help|can't help)\b",
            r"\bi(?:'m| am) (?:sorry|not able|unable)\b",
            r"\bas an ai\b",
            r"\bas a language model\b",
            r"\bi don't have (?:the )?(?:ability|capability|permission)\b",
            r"\bnot (?:comfortable|appropriate) (?:for me )?to\b",
            r"\bi (?:can't|cannot) (?:assist|comply|provide|help) with\b",
        ]
    ...
    async def _maybe_join_conversation(self, message: discord.Message):
        ...
        if not self._user_spontaneous_enabled(str(message.author.id)):
            self._record_action_audit(message, action_type="skip", reason="user spontaneous responses disabled")
            return
    ...
    async def _forward_private_message(self, message: discord.Message) -> None:
        ...
        lines = [f"**DM to {self._bot_identity()} from {author_label}**"]
        if content:
    ...
    def _user_spontaneous_enabled(self, user_id: str) -> bool:
        if not self.action_audit:
            return False
        return bool(self.action_audit.get_user_privacy(user_id).get("spontaneous_enabled", False))
...
class MainCommands(commands.Cog):
    ...
    def __init__(
        ...
        self.admin_user_ids = set(x.strip() for x in admin_ids.split(",") if x.strip())
        if self.config:
            self.admin_user_ids.update(getattr(self.config, "control_admin_ids", []) or [])
        self.hermes_client = None
        if self.config and getattr(self.config, "hermes_enabled", False):
            self.hermes_client = HermesClient(
                binary=self.config.hermes_binary,
                profile=self.config.hermes_profile,
                provider=self.config.hermes_provider,
                model=self.config.hermes_model,
                timeout_seconds=self.config.hermes_timeout_seconds,
                max_output_chars=self.config.hermes_max_output_chars,
            )

    def _is_dev(self, ctx) -> bool:
        return str(ctx.author.id) in self.admin_user_ids
    ...
    def _command_help(self) -> dict:
        return {
            ...
            "help": {
                ...
                "details": "Examples: `!help`, `!help control`, `!help trigger`.",
            },
            "control": {
            ...
            "highlights": {
                "usage": "!highlights [scan_limit]",
                "summary": "Show one random high-signal message from the top recent reaction highlights in this channel.",
                "details": (
                    ...
                    "It randomly picks one message from the top 5 candidates and shows an embed preview. "
                    "`scan_limit` defaults to 10000."
                ),
            },
            "trigger": {
            ...
            "admin": {
                "usage": "!admin <status|ask|profiles|profile|logs|mark|labels|memories|style|topics|dashboard|topic|channel|learn|lastaction|why|clear>",
                ...
                "details": (
                    ...
                    "quality labels, channel controls, learning jobs, recent action logs, cleanup tools, "
                    "and the optional Hermes harness when HERMES_ENABLED=true."
                ),
            },
        }
    ...
    async def help_command(self, ctx, command_name: str = ""):
        ...
        embed.add_field(name="Details", value="Use `!help <command>` for details, like `!help trigger`.", inline=False)
    ...
    @staticmethod
    def _highlight_embed(channel_name: str, item: dict) -> discord.Embed:
        message = item["message"]
        content = item["content"].replace("\n", " ")
        if len(content) > 300:
            content = content[:297].rstrip() + "..."
        embed = discord.Embed(
            title=f"Highlight from #{channel_name}",
            description=content,
            color=discord.Color.gold(),
            timestamp=getattr(message, "created_at", None),
        )
        embed.add_field(name="Author", value=item["author"], inline=True)
        embed.add_field(name="Reactions", value=str(item["score"]), inline=True)
        jump_url = getattr(message, "jump_url", "")
        if jump_url:
            embed.add_field(name="Original", value=f"[Jump to message]({jump_url})", inline=False)
        return embed

    @commands.command(name="highlights", aliases=["highlight"])
    async def highlights(self, ctx, scan_limit: int = 10000):
        scan_limit = max(1, min(10000, scan_limit))
        candidates = []
        ...
        await ctx.send(embed=self._highlight_embed(ctx.channel.name, item))
    ...
    async def trigger(self, ctx, *, args: str = ""):
        ...
        if not trigger_text:
            lines = [
                "Usage: `!trigger <text> <response>` or attach an image with `!trigger <text>`.",
                "Use quotes for multi-word triggers, like `!trigger \"good bot\" thanks`.",
            ]
            if guild_id:
                triggers = self.profiles.list_triggers(guild_id=guild_id, limit=10)
                if triggers:
                    lines.append("\nRecent triggers:")
                    for item in triggers:
                        lines.append(f"- `{item['trigger_text']}` -> `{item['reaction']}`")
            await ctx.send("\n".join(lines)[:1900])
            return
        if not response:
            await ctx.send(
                "Missing response. Use `!trigger <text> <response>` or attach an image with `!trigger <text>`."
            )
            return
        if not guild_id:
    ...
    async def admin(self, ctx, action: str = "status"):
        ...
        if action == "status":
            ...
            msg = [
                ...
                f"Admin IDs: {', '.join(self.admin_user_ids) or 'none'}",
            ]
            await ctx.send("\n".join(msg))

        elif action == "ask":
            await self._admin_ask_hermes(ctx)
    ...
    async def _admin_ask_hermes(self, ctx) -> None:
        if not self.hermes_client:
            await ctx.send("Hermes is not enabled. Set `HERMES_ENABLED=true` and configure the Hermes binary/profile.")
            return
        prompt = ctx.message.content.split(maxsplit=2)
        if len(prompt) < 3 or not prompt[2].strip():
            await ctx.send("Usage: `!admin ask <request for Hermes>`")
            return
        request = prompt[2].strip()
        async with ctx.channel.typing():
            result = await self.hermes_client.ask(
                request,
                toolsets=getattr(self.config, "hermes_admin_toolsets", ""),
            )
        if result.ok:
            body = result.output or "(Hermes returned no output.)"
        else:
            body = f"Hermes failed with code {result.returncode}.\n{result.error or result.output or 'No error output.'}"
        for chunk in self._discord_chunks(body):
...
class ControlCommands(commands.Cog):
    ...
    def _self_control_usage(self) -> str:
        return (
            ...
            "`!control spontaneous on|off`, `!control remember on|off`, or `!control forget me`."
        )
    ...
    async def _handle_self_control(self, ctx, action: str, target: str = "") -> None:
        ...
        if action in {"status", "me"} and not target:
            ...
            spontaneous = "on" if privacy.get("spontaneous_enabled", False) else "off"
            await ctx.send(
                f"{self._self_mode_description(mode)}\n"
                f"Spontaneous replies/reactions to me: `{spontaneous}`.\n"
                f"Remember me: `{remember}`.\n"
                f"{self._self_control_usage()}"
            )
            return
        ...
        if action == "spontaneous":
            ...
            if value not in {"on", "off"}:
                await ctx.send("Usage: `!control spontaneous <on|off>`")
                return
            self.action_audit.set_user_spontaneous_enabled(user_id, value == "on")
            await ctx.send(f"Spontaneous replies/reactions to you set to `{value}`.")
            return
        ...
        if action == "privacy":
            ...
            await ctx.send(
                ...
                f"Spontaneous: `{'on' if privacy.get('spontaneous_enabled', False) else 'off'}`. "
                f"Remember me: `{'on' if privacy.get('remember_enabled', True) else 'off'}`."
            )
            return
