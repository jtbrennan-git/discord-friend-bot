"""
Test suite for Discord Friend Bot.
"""
...
from utils.hermes import HermesClient, HermesResult
...
class TestHighlights:
    ...
    def test_highlight_embed_previews_one_message(self):
        candidate = self.commands._highlight_candidate(self._message("that track goes hard", [2, 3]))

        embed = self.commands._highlight_embed("music", candidate)

        assert embed.title == "Highlight from #music"
        assert embed.description == "that track goes hard"
        assert embed.fields[0].name == "Author"
        assert embed.fields[0].value == "Alice"
        assert embed.fields[1].name == "Reactions"
        assert embed.fields[1].value == "5"
        assert "Jump to message" in embed.fields[2].value
...
class TestResponseSending:
    ...
    def setup_method(self):
        ...
        self.bot.bot.user.display_name = "Discord Friend Bot"
    ...
    @pytest.mark.asyncio
    async def test_direct_mention_command_is_processed_with_clean_content(self):
        self.bot.config.command_prefix = "!"
        self.bot.bot.process_commands = AsyncMock()
        message = MagicMock()
        message.content = "<@42> !highlight"

        handled = await self.bot._maybe_process_direct_command(message, "!highlight")

        assert handled is True
        self.bot.bot.process_commands.assert_awaited_once_with(message)
        assert message.content == "<@42> !highlight"

    @pytest.mark.asyncio
    async def test_refusal_like_reply_is_retried_with_context(self):
        self.bot.llm_client = LLMClient(LLMConfig(api_key="test"))
        self.bot._llm_generate = AsyncMock(return_value="[REPLY] yeah, that's a weird ask")

        parsed = await self.bot._retry_refusal_reply(
            original_content="say something normal",
            refused_text="I can't help with that.",
        ...
        assert parsed.content == "yeah, that's a weird ask"
        retry_prompt = self.bot._llm_generate.call_args.args[0]
        assert "generic assistant refusal" in retry_prompt
        assert "casual Discord friend-group context" in retry_prompt

    @pytest.mark.asyncio
    async def test_retry_refusal_falls_back_to_silent_if_still_refusing(self):
        self.bot.llm_client = LLMClient(LLMConfig(api_key="test"))
        self.bot._llm_generate = AsyncMock(return_value="[REPLY] As an AI, I can't help with that.")
    ...
    @pytest.mark.asyncio
    async def test_spontaneous_join_is_user_opt_in(self):
        self.bot.action_audit = MagicMock()
        self.bot.action_audit.get_user_privacy.return_value = {"spontaneous_enabled": False}
        self.bot._channel_controls = MagicMock(return_value={"quiet_enabled": False, "spontaneous_react_enabled": True})
        self.bot._record_action_audit = MagicMock()
        self.bot.channel_states = MagicMock()
        self.bot.channel_states.get.return_value = MagicMock()
        message = MagicMock()
        message.author.id = "u1"
        message.channel.id = "c1"

        await self.bot._maybe_join_conversation(message)

        self.bot._record_action_audit.assert_called_once_with(
            message,
            action_type="skip",
            reason="user spontaneous responses disabled",
        )
...
class TestActionAuditStore:
    ...
    def test_user_privacy_controls(self):
        privacy = self.store.get_user_privacy("u1")
        assert privacy["remember_enabled"] is True
        assert privacy["spontaneous_enabled"] is False

        self.store.set_user_remember_enabled("u1", False)
        self.store.set_user_spontaneous_enabled("u1", True)

        privacy = self.store.get_user_privacy("u1")
        assert privacy["remember_enabled"] is False
        assert privacy["spontaneous_enabled"] is True
...
class TestHermesHarness:
    def test_build_command_uses_arg_list(self):
        client = HermesClient(
            binary="hermes",
            profile="discord-bot",
            provider="openrouter",
        ...
        cmd = client.build_command("hello; rm -rf /", "WEB,image_gen")
    ...
    @pytest.mark.asyncio
    async def test_admin_ask_requires_enabled_hermes(self):
        config = MagicMock()
        config.hermes_enabled = False
        config.control_admin_ids = []
        commands_cog = MainCommands(
            bot=MagicMock(),
            llm_client=None,
            memory=None,
            config=config,
        ...
        await commands_cog._admin_ask_hermes(ctx)

        ctx.send.assert_awaited_once()
        assert "Hermes is not enabled" in ctx.send.call_args.args[0]

    @pytest.mark.asyncio
    async def test_admin_ask_sends_hermes_output(self):
        config = MagicMock()
        config.hermes_enabled = True
        config.control_admin_ids = []
        config.hermes_binary = "hermes"
        config.hermes_profile = "discord-bot"
        config.hermes_provider = "openrouter"
        config.hermes_model = "openrouter/owl-alpha"
        config.hermes_timeout_seconds = 60
        config.hermes_max_output_chars = 4000
        config.hermes_admin_toolsets = "all"
        commands_cog = MainCommands(
            bot=MagicMock(),
            llm_client=None,
            memory=None,
        ...
        commands_cog.hermes_client = MagicMock()
        commands_cog.hermes_client.ask = AsyncMock(return_value=HermesResult(ok=True, output="done"))
        ctx = MagicMock()
        ctx.message.content = "!admin ask inspect this"
        ...
        commands_cog.hermes_client.ask.assert_awaited_once_with(
            "inspect this",
            toolsets="all",
        )
        ctx.send.assert_awaited_once_with("done")

    def test_control_admins_are_admin_command_devs(self):
        config = MagicMock()
        config.hermes_enabled = False
        config.control_admin_ids = ["123"]
        commands_cog = MainCommands(
            bot=MagicMock(),
            llm_client=None,
            memory=None,
        ...
        assert commands_cog._is_dev(ctx) is True
...
class TestFeedbackTracker:
    ...
    def test_lolll_custom_emoji_is_positive(self):
        self.tracker.register_message("1", "test message")
        self.tracker.add_reaction("1", "<:lolll:12345>", "user1")
        assert self.tracker.total_positive == 1
