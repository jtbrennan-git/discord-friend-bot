...
DB_PATH = os.getenv("ACTION_AUDIT_DB_PATH", "/tmp/bot_actions.db")
...
class ActionAuditStore:
    ...
    def _init_db(self) -> None:
        with closing(sqlite3.connect(self.db_path)) as conn:
            ...
            privacy_columns = {
                row[1]
                for row in conn.execute("PRAGMA table_info(user_privacy_controls)").fetchall()
            }
            if "spontaneous_enabled" not in privacy_columns:
                conn.execute(
                    "ALTER TABLE user_privacy_controls ADD COLUMN spontaneous_enabled INTEGER NOT NULL DEFAULT 0"
                )
            conn.execute(
                """
    ...
    def get_user_privacy(self, user_id: str) -> Dict[str, bool]:
        with closing(sqlite3.connect(self.db_path)) as conn:
            row = conn.execute(
                "SELECT remember_enabled, spontaneous_enabled FROM user_privacy_controls WHERE user_id = ?",
                (str(user_id),),
            ).fetchone()
        return {
            "remember_enabled": bool(row[0]) if row else True,
            "spontaneous_enabled": bool(row[1]) if row else False,
        }
    ...
    def set_user_spontaneous_enabled(self, user_id: str, enabled: bool) -> None:
        with closing(sqlite3.connect(self.db_path)) as conn:
            conn.execute(
                """
                INSERT INTO user_privacy_controls (user_id, spontaneous_enabled)
                VALUES (?, ?)
                ON CONFLICT(user_id) DO UPDATE SET
